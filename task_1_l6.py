...
# Также следует отметить, что вначале работы программы new_num и num имеют иные размер памяти,
# но в сумме меньшие, чем в конце.
...
#
# Так как в данной программе фиксированные размеры массивов и в массивы не будут попадать нули
# в качестве значений, объем памяти будет всегда одинаковым - 386 байт.

# Исходя из данных, программы с наиболее эффективным использованием памяти - это первая и третья,
# так как их потребление памяти будет константным и предсказуемым за счет заранее определенных размеров переменных
# (в первой - число из трех цифр, во второй - фиксированный размер массива).
